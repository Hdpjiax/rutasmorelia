import {selectInitialJourneyRouteId, type Coordinates} from '@rutas-morelia/transit-core';
...
function GeometryProbe({camera}: {camera: React.RefObject<CameraRef | null>}) {
...
function resetStores() {
  ...
  useUiStore.setState({routeGeometryLoading: false, message: null, toastKind: 'info'});
}
...
describe('TransitMap geometry flow (shipped hook + components)', () => {
  ...
  it('planJourney → activateRoute → useRouteGeometry loads geojson into RouteLayers', async () => {
    ...
    expect(useTransitStore.getState().activeRouteId).toBeTruthy();

    const camera = createRef<CameraRef | null>();
    await render(
      <ThemeProvider>
        <GeometryProbe camera={camera} />
      </ThemeProvider>,
    );
