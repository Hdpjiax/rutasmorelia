...
import type {Coordinates, FavoriteItem, Suggestion} from '../types/transit';
...
export async function resolveSuggestionCoords(
  ...
  try {
    ...
    if (__DEV__) console.warn('[ViaMorelia] Failed to resolve suggestion coords:', e);
    return null;
  }
}
...
export async function searchPlaceByName(
  ...
  try {
    ...
    if (__DEV__) console.warn('[ViaMorelia] Database fallback search failed:', e);
    return null;
  }
}